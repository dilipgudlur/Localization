...
class SocketStream implements NetworkFrameStream {
  ...
  public synchronized Frame recvFrame() {
    try {
      Frame f = recvBuffer.take();  // retrieve frame from receive buffer (will block until one is available)
      return f;
    }
    ...
    public void run() {
      try {
        ...
        if (message instanceof Frame) {
          ...
          System.out.println("incoming " + ((Frame) message).seqNum);
        }
        ...
        System.out.println("FrameHandler done.");
      }
      catch (IOException e) { System.out.println("Error, connection closed abnormally: " + e); }
