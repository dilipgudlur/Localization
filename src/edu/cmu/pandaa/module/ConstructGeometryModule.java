...
public class ConstructGeometryModule implements StreamModule {
	...
	public ConstructGeometryModule(int argLen)
	{
		this.argLen = argLen;
	}
	...
	public StreamHeader init(StreamHeader inHeader) {
	    ...
	    DistanceHeader[] distanceHeaders = (DistanceHeader[]) multiHeader.getHeaders();
	    /*if (distanceHeaders.length != 3) {
	      throw new IllegalArgumentException("Invalid number of elements");
	    }*/
	    
	    /*frametime check for each pair of distanceHeader*/
	    for(int i=0;i<argLen-1;i++){
	    	for(int j=i+1;j<argLen-1;j++){
	    		if(distanceHeaders[i].frameTime != distanceHeaders[j].frameTime);
	...
	public static void main(String[] args) throws Exception {
	    ...
	    System.out.println("Combine "+(argLen - 1)+" pairwise distances ");
	    for(i=0;i<argLen-1;i++){
	    	System.out.println(inArg[i]+", ");
	    }
	    ...
	    ConstructGeometryModule ppd = new ConstructGeometryModule(args.length);
